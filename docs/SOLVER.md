...
- `--epsilon=E` (alias `--kappa0=E`) solves for
  ...
  $\varepsilon = 1.1$. The run prints the coefficient, its range and its
  contrast on the first line, so a log is self-describing. Values $\le 0$ are
  rejected with an error rather than silently producing a degenerate operator.
